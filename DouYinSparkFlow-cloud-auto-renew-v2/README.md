...
仓库已包含 GitHub Actions 定时工作流。Fork 后，在仓库的 `Settings -> Environments` 创建 `user-data`，将普通配置放入 Environment variables，将每个 `COOKIES_<UNIQUE_ID>` 放入 Environment secrets。首次请在 `Actions` 手动运行一次，确认日志中每个目标好友均显示“已发送续火花消息”。

工作流默认每天北京时间 03:00 运行；时间由 `.github/workflows/schedule.yml` 中的 UTC cron 表达式控制。请合理控制运行频率，并在 Cookies 失效或页面结构变化时及时查看 `run-logs`。
